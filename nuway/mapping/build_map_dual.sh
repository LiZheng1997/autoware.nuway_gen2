...
ros2 launch /home/lz/campus_map/mapping_nuway_dual.launch.yaml > "$D/fastlio.log" 2>&1 &
sleep 10

log "收集器 + 轨迹记录"
python3 /home/lz/collect_map.py "$D/campus_dual.pcd" > "$D/collect.log" 2>&1 &
CP=$!
...
tail -1 "$D/traj.log"
ls -la "$D/campus_dual.pcd" 2>/dev/null
echo "DONE_DUAL $(date +%T)"
